...
def init_model(model_type,args):
    ...
    if model_type == 'LeNet5':
        ...
        model = Model.ResNet18(numcls=args.classes)
    elif model_type == 'CNN':
        model = Model.CNN()
...
class Node(object):
    def __init__(self, num, train_data, test_data, args):
        ...
        self.model = init_model(self.args.local_model,args=self.args).to(self.device)
...
class Global_Node(object):
    def __init__(self, test_data, args):
        ...
        self.args = args
        self.device = self.args.device
        self.model = init_model(self.args.global_model,args=self.args).to(self.device)

        self.test_data = test_data
        self.Dict = self.model.state_dict()

#         self.edge_node = [Model.ResNet18().to(self.device) for k in range(args.node_num)]
        self.edge_node = [init_model(self.args.global_model,args=self.args).to(self.device) for k in range(args.node_num)]
        self.init = False
        self.save = []
    ...
    def processing(self):
        ...
        for key in self.Dict.keys():
            if 'num_batches_tracked' in key:
                continue
            for i in range(1, self.args.node_num): #we can change here to 60%
            #for i in range(1, 0.6*self.args.node_num):
                self.Dict[key] += Node_State_List[i][key]
            # self.Dict[key] = self.Dict[key].float()     # 不知道为什么数据类型会发生变化
