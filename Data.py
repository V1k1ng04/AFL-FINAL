...
class FEMNIST(MNIST):
    ...
    def download(self):
        ...
        if not os.path.exists(self.raw_folder): os.makedirs(self.raw_folder)
...
def Dataset(args):
    ...
    if args.dataset == 'femnist' or 'mnist':
        ...
        if args.dataset == 'femnist':
            trainset = FEMNIST(root='./data', train=True, transform=tra_trans,download=True)
            testset = FEMNIST(root='./data', train=False, transform=val_trans,download=True)
...
class Data(object):
    ...
    def __init__(self, args):
        ...
        self.test_all = DataLoader(testset, batch_size=args.batchsize, shuffle=True) #, num_workers=1) #changed num worker
