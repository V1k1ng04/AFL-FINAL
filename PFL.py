...
for rounds in range(args.R): 
    ...
    logger.write_new(round = rounds+1, test_acc=recorder.val_acc[str(Global_node.num)][rounds], curr_elapsed_time=round((time.time() - timerStart), 2))
    # logger.write(rounds=rounds + 1, test_acc=recorder.val_acc[str(Global_node.num)][rounds])


totaltime = round((time.time() - timerStart), 2)
logger.writetime(totaltime)
recorder.finish()
logger.close()
